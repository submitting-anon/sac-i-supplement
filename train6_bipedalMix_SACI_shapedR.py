...
from sac_vanilla import SAC_I, PolicyNetwork, create_base_dir, load_model_weights, \
                        ScoreBoard_bipedalH3
...
class MultiAgentSACTrainer:
    ...
    def update(self, batch_size):
        ...
        if self.policy_network_type == PolicyNetworkType.STANDARD:
            ...
            # states, actions, rewards, rewards_I, next_states, dones, state_trajs, _ = self.replay_buffers.sample(self.batch_size)
            states, actions, rewards, rewards_I, next_states, use_I, dones, state_trajs, _ = self.replay_buffers.sample(self.batch_size)
    
        # alphaK = 0.003 # fixed/constant alpha (tuning off)
        alphaK = None
        ...
        try:
            ...
            for episode in range(self.max_num_episodes):
                ...
                for step_num in range(self.max_num_steps_per_episode):
                    ...
                    if (step_num+1) % 10 == 0:
                        ...
                        if sum(last_rewards) < 0 and step_num > 20: # April19: skip initial stuck phase
                            ...
                            count_stop+=1
                            use_I = 1
                            if not got_stuck_previous_step:
                                # border up: it is the first step to get stuck
                                cum_stuck_penalty = 0 # reset
                    ...
                    if use_I:
                        if reward == -100: # disconsider hit in Q_I to motivate exploring more...
                            reward_I = sum(last_rewards) # seeds >=4
                        else:
                            reward_I = reward # env reward 
                            reward_I += sum(last_rewards) # add last 5 steps cum reward (if stuck, usually it is negative...) the idea is to motivate to get out of stuck position
                            
                            cum_stuck_penalty += reward
                        
                        got_stuck_previous_step = True
                    else:
                        if (got_stuck_previous_step) and (not NO_BONUS):
                            # border down extra reward: it got out of stuck condition
                            reward_I = np.abs(cum_stuck_penalty)
                            
                            print('(***) Got out of stuck position!!! Got reward:',np.abs(cum_stuck_penalty))
                            cum_stuck_penalty = 0 # reset
                        else:
                            reward_I = reward
                    ...
                    if self.policy_network_type == PolicyNetworkType.STANDARD:
                        nfactor = 1
                        self.replay_buffers.push(state, action, reward/nfactor, reward_I/nfactor, use_I, next_state, done01) #
                ...
                if ready_to_train and log_interval == 0:
                    ...
                    # Update scoreboard
                    if episode % 1 ==0:
                        self.scoreboard.update_data_frame(current_total_num_timesteps,episode,average_alpha,average_alpha_I,average_loss,nUpdates,cum_return,list_rewards,list_epilen,nhits,(count_I/step_num),trialtype)
