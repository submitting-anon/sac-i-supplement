##################################################################################
# 3) Script to train SAC-I agent
# - Env: LunarLanderContinuous-v2
# - Reward: 
#     r_R = r_env + r_time + r_bomb
#     r_I = r_env + r_time + r_bomb + r_shapedC (very similar reward to the SAC)
# - State: Add bomb coordinates if it appears
# - Inhibition rule: this agent used Q-I whenever the bomb appeared (agressive inhibition)
# - Q-I weights: loaded from the baseline agent
# - Options:
...
from sac_vanilla import SAC_I, PolicyNetwork, create_base_dir, load_model_weights, ScoreBoard_bombs
...
class MultiAgentSACTrainer:
    ...
    def update(self, batch_size):
        ...
        alphaK = None
        policy_losses, alphas, alphas_I, td_errors, q_targets = self.sac.learn_both_splitAlpha_splitQloss(
        ...
        try:
            ...
            for episode in range(self.max_num_episodes):
                ...
                for step_num in range(self.max_num_steps_per_episode):
                    ...
                    reward_I = reward + reward_bomb #
                    
                    # shapedC: bomb proximity
                    isClose = False
                    if (y_next>y_bomb[0]) and (y_next<y_bomb[1]+0.4):
                        
                        d2bomb = dist_euclidean([xc,yc],[x_next,y_next])
                        k_dist = 0.3 # (min ~-20) # SAC DEFAULT...
                        if d2bomb < k_dist:
                            penalty_too_close = -10000*(d2bomb-k_dist)*(d2bomb-k_dist)*(d2bomb-k_dist)*(d2bomb-k_dist) #  min~-27 (k_dist = 0.3) 
                            reward_I += penalty_too_close
                            r_too_close += penalty_too_close
                            isClose = True
                            
                    ## Inhibition Rule
                    if bombAppeared: # Simplified
                        use_I = True
                        # print('too close penalty (use_I):',penalty_too_close)
                        # print('reward:',reward)
                ...
                if ready_to_train and log_interval == 0:
                    ...
                    # Update scoreboard
                    if episode % 1 ==0:
                        # self.scoreboard.update_data_frame(current_total_num_timesteps,episode,average_alpha,average_loss,nUpdates,list_rewards,list_epilen,nhits,nbombs,(count_I/step_num))
